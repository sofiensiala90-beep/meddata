...
const Header: React.FC<HeaderProps> = ({ toggleSidebar }) => {
  ...
  const { notifications, handleMarkNotificationsRead } = useData();
  ...
  const handleBellClick = () => {
    ...
    if (!isNotificationsOpen && unreadCount > 0) {
      handleMarkNotificationsRead(currentUser.id);
    }
  };
  ...
  const title = pageTitles[currentPath] || 'MedataAI';
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          ...
          {currentUser.role === 'student' && (
            <div className="hidden sm:flex items-center space-x-2 bg-slate-100 dark:bg-slate-700/50 px-3 py-1.5 rounded-full text-sm font-medium text-slate-700 dark:text-slate-200" title="Votre solde de Coins">
              ...
              <span className={`font-semibold ${currentUser.coinBalance < 0 ? 'text-red-500' : ''}`}>{currentUser.coinBalance.toLocaleString()}</span>
            </div>
          )}
