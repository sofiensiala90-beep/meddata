...
const AuthPage: React.FC<AuthPageProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user && !isCompletingGoogleSignup) {
        setSignupData(prev => ({
          ...prev,
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCompletingGoogleSignup) {
        ...
        if (!user) {
          console.error("Session perdue lors de l'inscription Google");
          throw new Error("Erreur de session. Veuillez recharger la page et réessayer.");
        }
      } else {
      ...
      if (user) {
        ...
        try {
          ...
          console.warn("Erreur lors de la création des notifications/activités (non bloquant):", secondaryError);
        }
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        setSignupError("Erreur de permission. Veuillez contacter le support.");
      } else {
        setSignupError(`Erreur lors de la création du compte : ${error.message || error}`);
      }
    } finally {
      setIsLoading(false);
    }
